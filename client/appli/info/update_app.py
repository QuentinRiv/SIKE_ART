...
from appli.tree.pynode import compute_leaf_keys, fill_nodes, create_tree
from appli.tree.pynode import get_node, add_node, lowest_common_node, remove_node
...
from appli.message.cryptom import get_key_aes, decrypt_aes
...
def check_db():
    ...
    for i in order_time:
        if all_data[i] in mails:     # If it is a message
            ...
            # or a group one
            keyy = get_key_aes(
                mail['sender'], mail['recipient'], info_otk=mail['info_otk'])
