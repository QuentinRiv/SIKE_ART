...
from appli.info.mysike import eA, eB, p, createSS
from appli.info.mysike import compute_pk, Complex, server_address
from appli.message.cryptom import X3DH
from appli.info.db_func import get_otk, get_priv_id, get_token
    ...
    if not node.isLeaf:
        if node.index in tofill:
            ...
            if node.children[1].private_key:
                ...
                # print("On connait la pK de ",
                #       node.children[1].value, ":",
                # node.children[1].public_key)
                secret_key = createSS(node.children[1].public_key,
                                      node.children[0].private_key,
                                      side='initiator', method=method)
            else:
                raise Exception("A private key is missing for {}"
                                .format(node.index))
