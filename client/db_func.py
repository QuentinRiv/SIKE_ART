...
# from cryptom import decrypt_aes, get_key_aes
...
def get_stagekey(groupname):
    group = Group.query.filter_by(owner=current_user.name, groupname=groupname).first()
    if not group:
...
def register_db(name, public_id, side):
    ...
    party = {"owner": name, "publicID": repr(public_id), 'side': side}
...
def get_mail_db(user, partner):
    ...
    if partner in groupnames:   # If it's a group
        ...
        group = Group.query.filter_by(owner=current_user.name, groupname=recipient).first()
